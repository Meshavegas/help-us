...
export async function GET(
  ...
  try {
    ...
    const apiResponse = await makeAuthenticatedRequest(`/users/${id}`);
    const data = await apiResponse.json();

    if (!apiResponse.ok) {
      return NextResponse.json(
        { error: data.message || 'Erreur lors de la récupération de l\'utilisateur' },
...
export async function PUT(
  ...
  try {
    ...
    if (!apiResponse.ok) {
      return NextResponse.json(
        { error: data.message || 'Erreur lors de la mise à jour de l\'utilisateur' },
        { status: apiResponse.status }
      );
    }
